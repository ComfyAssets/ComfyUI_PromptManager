...
class ImageRoutesMixin:
    ...
    async def serve_image(self, request):
        ...
        try:
            ...
            return web.json_response({'success': False, 'error': 'Invalid image ID'}, status=400)
        except Exception as e:
    ...
    async def serve_output_image(self, request):
        ...
        try:
            ...
            output_dir = self._find_comfyui_output_dir()
            if not output_dir:
                return web.json_response({'success': False, 'error': 'ComfyUI output directory not found'}, status=404)
    ...
    async def generate_thumbnails(self, request):
        ...
        try:
            ...
            result = await self._run_in_executor(
                ...
                output_path, thumbnails_dir, thumbnail_size
            )
    ...
    async def link_image_to_prompt(self, request):
        ...
        try:
            ...
            image_id = await self._run_in_executor(self.db.link_image_to_prompt, prompt_id, image_path, metadata)
    ...
    async def get_image_prompt(self, request):
        ...
        try:
            ...
            try:
                prompt_data = await self._run_in_executor(self.db.get_image_prompt_info, image_path)
                if prompt_data:
