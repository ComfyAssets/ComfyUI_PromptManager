...
This module replaces the disaster 2,179-line __init__.py with a clean,
...
class PromptManagerAPI:
    ...
    def __init__(self, comfyui_root: Path = None):
        ...
        self.comfyui_root = comfyui_root or Path.cwd()
    ...
    def add_routes(self, routes):
        ...
        routes.add_route('GET', '/api/prompt_manager/prompts', 
                        self.prompt_handlers.list_prompts)
        routes.add_route('POST', '/api/prompt_manager/prompts', 
                        self.prompt_handlers.create_prompt)
        routes.add_route('GET', '/api/prompt_manager/prompts/{id}', 
                        self.prompt_handlers.get_prompt)
        routes.add_route('PUT', '/api/prompt_manager/prompts/{id}', 
                        self.prompt_handlers.update_prompt)
        routes.add_route('DELETE', '/api/prompt_manager/prompts/{id}', 
                        self.prompt_handlers.delete_prompt)
        
        # ========== SEARCH & FILTERING ==========
        routes.add_route('GET', '/api/prompt_manager/search', 
                        self.prompt_handlers.search_prompts)
        routes.add_route('GET', '/api/prompt_manager/duplicates', 
                        self.prompt_handlers.find_duplicates)
        
        # ========== SYSTEM MANAGEMENT ==========
        routes.add_route('GET', '/api/prompt_manager/health', 
                        self.system_handlers.health_check)
        routes.add_route('GET', '/api/prompt_manager/stats', 
                        self.system_handlers.get_stats)
        routes.add_route('POST', '/api/prompt_manager/backup', 
                        self.system_handlers.create_backup)
        routes.add_route('POST', '/api/prompt_manager/restore', 
                        self.system_handlers.restore_backup)
        routes.add_route('POST', '/api/prompt_manager/vacuum', 
                        self.system_handlers.vacuum_database)
        ...
        async def serve_web_ui(request):
            ...
            try:
                ...
                # Build full path
                web_dir = self.comfyui_root / "custom_nodes" / "ComfyUI_PromptManager" / "web"
