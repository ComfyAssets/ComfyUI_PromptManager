...
from ..validation.file_ops import AtomicWriter, DirectoryManager, FileCleanup  # type: ignore
from ..validation.hashing import CacheKeyGenerator  # type: ignore

logger = get_logger("promptmanager.cache")


def _get_cache_base_dir() -> Path:
    """Get the base directory for cache storage using ComfyUI root detection."""
...
class DiskCache:
    ...
    def __init__(
        ...
        """
        self.cache_dir = Path(cache_dir) if cache_dir else _get_cache_base_dir() / '.cache'
        self.max_size = max_size
        self.default_ttl = default_ttl
...
def init_default_caches():
    ...
    image_cache = MultiLevelCache(
        ...
        cache_dir=_get_cache_base_dir() / '.cache' / 'images'
    )
    ...
    api_cache = DiskCache(
        cache_dir=_get_cache_base_dir() / '.cache' / 'api',
